...
private:
  ...
  void init_window() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, "jubaengine", NULL, NULL);
  }
  ...
  void create_sync_objects() {
    ...
    if (vkCreateSemaphore(device,&semaphore_info, nullptr, &image_available_semph) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create the image_available_semph");
    }
    if (vkCreateSemaphore(device, &semaphore_info, nullptr, &render_finished_semph) != VK_SUCCESS) {
  ...
  void draw_frame() {
      ...
      vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, image_available_semph, VK_NULL_HANDLE, &image_index);
      vkResetCommandBuffer(command_buffer, 0);
      record_command_buffer(command_buffer, image_index);
      VkSubmitInfo submit_info{};
      submit_info.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
      VkSemaphore wait_semaphores[] = {image_available_semph};
      ...
      VkSemaphore signal_semaphores[] = {render_finished_semph};
  ...
  void create_graphics_pipeline() {
      ...
      input_assm_info.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
      ...
      if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &graphics_pipeline_info, nullptr, &graphics_pipeline) != VK_SUCCESS) {
  ...
  void create_swapchain() {
    ...
    if (swapchain_support.capabilities.maxImageCount > 0) {
      image_count =
          std::min(image_count, swapchain_support.capabilities.maxImageCount);
    }
  ...
  void cleanup() {
    vkDestroySemaphore(device, image_available_semph, nullptr);
    vkDestroySemaphore(device, render_finished_semph, nullptr);
  ...
  void create_logical_device() {
    ...
    // Retrieve the handle for the present_queue. (Useful for surface creation.)
